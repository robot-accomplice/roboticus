...
| SYS-13-003 | P2 | Transport metadata preservation differs by adapter and needs explicit classification | Rust per-channel metadata mapping needs comparison | The live adapters now preserve a more coherent normalized baseline: Telegram emits `is_group`, `chat_type`, and `sender_username`; Signal emits `is_group`; Discord emits `is_group` plus `guild_id`; WhatsApp emits `is_group`, `sender_phone`, and `message_type`; Matrix preserves `room_id`, `sender_mxid`, and now authoritative `is_direct` when the homeserver exposes `m.direct` account data. Full Matrix room-vs-group classification is still open because absence of `m.direct` is not an authoritative group signal. | Improved, not closed | Open | `internal/channel/telegram.go`, `internal/channel/signal.go`, `internal/channel/discord.go`, `internal/channel/discord_gateway.go`, `internal/channel/whatsapp.go`, `internal/channel/matrix.go`, coverage tests |
...
- 2026-04-17: Matrix now preserves explicit normalized identifiers
  (`room_id`, `sender_mxid`) instead of leaving all transport context implicit.
  DM-vs-room classification is still open, but the adapter no longer drops the
  basic Matrix-specific identity surface.
- 2026-04-18: Matrix now also preserves authoritative `is_direct` when the
  homeserver exposes `m.direct` account data on the sync path. This improves
  DM detection without overclaiming a full `is_group` classification where no
