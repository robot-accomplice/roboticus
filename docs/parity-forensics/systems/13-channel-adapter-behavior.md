...
| SYS-13-003 | P2 | Transport metadata preservation differs by adapter and needs explicit classification | Rust per-channel metadata mapping needs comparison | The live adapters now preserve a more coherent normalized baseline: Telegram emits `is_group`, `chat_type`, and `sender_username`; Signal emits `is_group`; Discord emits `is_group` plus `guild_id`; WhatsApp emits `is_group`, `sender_phone`, and `message_type`; Matrix now preserves explicit `room_id` and `sender_mxid` metadata. Matrix DM-vs-room classification is still open because the normalized surface does not yet carry authoritative direct-chat semantics. | Improved, not closed | Open | `internal/channel/telegram.go`, `internal/channel/signal.go`, `internal/channel/discord.go`, `internal/channel/discord_gateway.go`, `internal/channel/whatsapp.go`, `internal/channel/matrix.go`, coverage tests |
