...
| SYS-02-006 | P2 | Tool-surface divergence likely includes more than ranking defaults | Desired behavior is not just "same top-k knobs," but a coherent tool surface across registry descriptors, request injection, and runtime reuse | Go now has registry-time descriptors, descriptor embeddings, and query-time `SelectToolDefs(...)`; the remaining audit work is to prove these are the authoritative live surface rather than only one layer of it | Open | Open | `internal/agent/tools/registry.go`, `internal/agent/tools/prune.go`, `internal/daemon/daemon_adapters.go` |
