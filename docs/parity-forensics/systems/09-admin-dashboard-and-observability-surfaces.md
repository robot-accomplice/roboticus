...
| SYS-09-005 | P2 | Some trace search behavior is still implemented via ad hoc `LIKE` over serialized JSON rather than a more disciplined query surface | Observability search should stay truthful and evolvable as trace structure changes | `SearchTraces` filters `stages_json` / `react_trace_json` with `LIKE`, which is serviceable but format-coupled and potentially brittle as trace structure evolves | Degradation seam | Open | `internal/api/routes/traces.go:37-71` |
