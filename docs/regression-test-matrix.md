...
| R-BEH-27 | Runtime filesystem context tells the model that allowed paths are inherited roots, and short referential execution follow-ups such as "examine it" are expanded with previous assistant context so allowed vault subtrees do not turn into false configuration blockers | `internal/agent/tools/builtins_test.go::TestRuntimeContextTool`, `internal/pipeline/heuristics_test.go::TestContextualizeShortFollowup_ReferentialExecutionTask` | L1/L4 |
