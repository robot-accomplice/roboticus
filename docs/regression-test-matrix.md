...
| R-RT-26 | CLI command topology is reviewed and collapsed into intuitive operator categories for v1.0.8, with backward-compatible aliases retained for existing scripts | planned CLI taxonomy tests | L1/L2 |
| R-RT-27 | Ollama baseline failures classify local runtime/model availability separately from model efficacy, so unloaded or transport-failing local models do not appear as exercised all-zero quality cohorts in comparison tables | planned Ollama baseline classification tests | L1/L2/L4 |
