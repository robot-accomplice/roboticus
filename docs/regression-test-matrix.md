...
| R-SOAK-PATHS-1 | Clone-mode soak prep widens `allowed_paths` / `tool_allowed_paths` / `script_allowed_paths` in the ISOLATED config (operator's live config is untouched) so scenarios like `filesystem_count_only` that legitimately need to read user dirs (`~/code`, `~/Downloads`) succeed under the cloned policy | `scripts/run-agent-behavior-soak.py::extend_allowed_paths_for_soak` (audit) | A |
