...
| R-BEH-27 | Runtime filesystem context tells the model that allowed paths are inherited roots, short referential execution follow-ups such as "examine it" are expanded with previous assistant context, and false "this child path must be added to the allowlist" claims are blocked unless backed by an actual sandbox denial | `internal/agent/tools/builtins_test.go::TestRuntimeContextTool`, `internal/pipeline/heuristics_test.go::TestContextualizeShortFollowup_ReferentialExecutionTask`, `internal/pipeline/guards_truthfulness_test.go::TestFilesystemDenialGuard_BlocksFalseAllowlistChildPathClaim` | L1/L4 |
| R-BEH-28 | Guard and verifier findings emit contract-shaped RCA evidence with phase, severity, precondition state, violation state, recovery action, recovery outcome, and confidence effect before any retry or block decision is made | `internal/pipeline/guard_contract_test.go`, `internal/pipeline/turn_diagnostics_test.go` | L1/L2 |
| R-BEH-29 | Built-in guards do not replace model output with fixed user-facing canned prose; hard findings are represented as structured violations, scoped retries, or structured failures through the normal response path | `internal/pipeline/guard_canned_fitness_test.go`, `internal/pipeline/guards_test.go` | L1/L4 |
| R-BEH-30 | Generic capability denials such as “I do not have tools/access” are not allowed after successful runtime/tool evidence unless a concrete tool, policy, sandbox, network, or provider result proves the limitation; unsupported capability denial is retried as a guard failure rather than shipped as the final answer | `internal/pipeline/guards_truthfulness_test.go` | L1/L2 |
| R-BEH-31 | Final answers cannot consist of promissory execution scaffolding such as “let me test/check that assumption” after only introspection/runtime-context evidence; a direct execution turn must either execute, report observed facts, or fail honestly with the blocking evidence | `internal/pipeline/guards_behavioral_test.go`, `internal/pipeline/guards_truthfulness_test.go` | L1/L2 |
| R-BEH-32 | Inspection-backed turns that are explicitly asked to list, report, or summarize directory/file entries must include concrete observed entries from successful inspection output instead of only saying that a list existed or that results were satisfactory | `internal/pipeline/guards_truthfulness_test.go`, focused live child-vault inspection proof | L1/L2/L4 |
| R-BEH-33 | Recovered hard-guard failures remain diagnosable: when a guard retry repairs the final answer, RCA may keep the turn `degraded`, but `primary_diagnosis` must identify the recovered guard violation instead of remaining blank | `internal/pipeline/turn_diagnostics_test.go`, focused live child-vault inspection proof | L1/L2/L4 |
| R-BEH-34 | Planned helper wrappers and dormant subsystem methods are not allowed to remain as dead code. Root-level CLI helpers must be called by live commands/tests, and daemon subsystem loops must be wired into startup or removed in favor of the canonical subsystem path. | `go test ./cmd ./internal/daemon -count=1`, source audit for `nolint:unused` planned/dormant markers | L1 |
...
| R-MEM-17 | Memory confidence influence is explicitly ternary: contradictions lower confidence (`-1`), absent or non-evidence memory is neutral (`0`), and relevant retrieved evidence can reinforce (`+1`). Missing memory and derivable in-turn answers are not proof failures, and derivable tool-output memories degrade out of active evidence over time. | `internal/session/verification_evidence_test.go`, `internal/pipeline/verifier_test.go`, `internal/agent/memory/wave5_test.go` | L1/L2 |
...
| R-RT-24 | OpenAI-compatible provider onboarding is metadata-driven and key-reference coherent: DeepSeek is available from bundled provider defaults, setup emits a valid DeepSeek profile, the setup wizard covers the bundled cloud-provider set, request URLs use the documented chat completions path, and daemon/status key resolution can consume the Rust-compatible `<provider>_api_key` secret while preserving legacy `provider_key:<name>` entries | `internal/core/core_comprehensive_test.go`, `cmd/configcmd/setup_test.go`, `internal/llm/coverage_boost_test.go`, `internal/llm/coverage_db_test.go`, `internal/api/routes/health_test.go` | L1/L2 |
| R-RT-25 | Local keystore CLI operations mutate the encrypted local keystore without requiring a running daemon, while preserving provider-name convenience (`deepseek` → `deepseek_api_key`) and legacy `provider_key:<name>` compatibility for removal/status | `cmd/updatecmd/keystore_rekey_test.go` | L1 |
| R-RT-26 | CLI root help is grouped into intuitive operator domains while every existing top-level command path remains callable for backward compatibility | `cmd/cli_contract_test.go::TestCLI_CommandGroups`, `cmd/cli_contract_test.go::TestCLI_TopLevelCommands` | L1/L2 |
| R-RT-27 | Ollama baseline failures classify local runtime/model availability separately from model efficacy, so unloaded or transport-failing local models do not appear as exercised all-zero quality cohorts in comparison tables | `internal/llm/exercise_models_test.go::TestExerciseModels_TransportErrorsAreValidityOnlyEvidence`, `internal/db/exercise_repo_test.go::TestExerciseScorecard_ExcludesValidityOnlyFailures`, `internal/db/exercise_repo_test.go::TestExerciseScorecard_ExcludesLegacyBlankZeroFailures`, `cmd/models/models_test.go::TestBuildComparisonRows_FreshValidityOnlyRunDoesNotCreateEvidenceCoverage`, focused live `ollama/phi4-mini:latest` and `ollama/qwen32b-tq` row proof | L1/L2/L4 |
...
| R-TOOLS-16 | Channel formatter fuzz targets stay stable on adversarial delimiter floods instead of timing out on malformed inline-markup scans, with explicit hostile WhatsApp seeds kept in the fuzz corpus | `internal/channel/formatter_fuzz_test.go`, `internal/channel/formatter_test.go` | L1/L4 |
...
| R-REL-08 | The repository-declared Go toolchain floor tracks the patched standard-library version required by the security gate, so CI `govulncheck` cannot fail on a vulnerable stdlib while local development silently passes on a newer auto-selected toolchain | `go test ./...` + `govulncheck ./...` in CI/release gate | L4 |
| R-REL-09 | CI/release workflows use Node 24-capable GitHub Action majors, so release-branch jobs do not carry known Node 20 deprecation warnings | workflow review gate | L4 |
| R-REL-10 | MCP stdio diagnostic regressions must use deterministic in-process helper children rather than shell pipelines so CI validates transport behavior, not runner-specific `/bin/sh` semantics | `go test ./internal/mcp -run 'Test(ConnectStdio_FailureSurfacesChildStderr|StdioTransport_ChildDiagnosticHandlesLargeStderr|StdioTransport_ChildDiagnosticEmptyOnSuccessfulRun|ConnectStdio_ContextDeadlineCancelsHungInitialize)$' -count=1` | L4 |
| R-REL-11 | Release execution can be replayed explicitly for an existing tag through the canonical workflow path instead of depending solely on the original tag-push event | workflow validation plus tag-bound dispatch proof | L4 |
| R-REL-12 | Release publication derives tag name, prerelease gating, asset upload, site-sync behavior, and release notification from one explicit release tag authority under both tag-push and workflow-dispatch execution | workflow validation plus successful rerun of the published-tag release path | L4 |
| R-REL-13 | Active CI/release workflows do not depend on floating security tool versions or third-party repository-dispatch/release actions for critical control flow; pinned CLI tooling and first-party `gh` commands remain replayable under both CI and release reruns | workflow validation plus green CI/release rerun on the hardened path | L4 |
| R-REL-14 | The release workflow self-evaluates the live published release object and emits a success/failure report instead of treating upstream green jobs as sufficient proof | successful release rerun plus workflow summary / notification evidence | L4 |
...
| R-UPGRADE-5 | Runtime provider resolution consumes the refreshable provider pack: operator config wins, `providers_file` fills missing providers/fields, and embedded bundled providers are only the final fallback | `internal/core/core_comprehensive_test.go` | L1 |
| R-UPGRADE-6 | `applySkillsUpdate(refreshConfig=false)` preserves per-file: a manifest-declared skill that exists locally is left untouched (no SHA check), while a manifest-declared skill that's missing locally is fresh-installed and SHA-verified in the same call | `cmd/updatecmd/update_parity_test.go` | L1 |
...
| R-MCP-DIAG-3 | `StdioTransport.ChildDiagnostic()` truncates stderr to the most recent 8KB and indicates truncation — chatty children can't blow up the daemon's memory, and the diagnostic-relevant tail is preserved even on slower CI runners because stderr-drain waits on transport-owned collector completion instead of a buffer-length heuristic | `internal/mcp/stdio_diagnostic_test.go` | L1 |
...
| R-TOOLS-19 | Focused inspection turns resolve allowed-root aliases such as `~` / “home folder” through the same inspection-target authority as absolute paths, using the live current-user home directory instead of machine-specific hardcoded paths, so home-directory distribution/listing requests do not degrade into conversational “I’ll check” replies with zero tool calls | `internal/pipeline/inspection_turn_test.go`, `internal/pipeline/task_synthesis_test.go`, `internal/pipeline/turn_policy_test.go` | L1/L2 |
| R-DELEG-02 | Delegation orchestration threads incomplete workflows back to inference and leaves the parent workflow in progress instead of falsely completing; regression tests use lifecycle-owned drained background workers so async persistence cannot race TempDir cleanup and fail CI spuriously | `internal/pipeline/delegation_orchestration_test.go` | L1 |
...
| R-BENCH-RCA-1 | Benchmark exercise rows persist row-level outcome class, so clean pass, slow pass, provider timeout, transport/API failure, empty response, and quality-gate failure are not collapsed into one generic pass/fail bucket. CLI/API result persistence carries the same classification used by RCA grouping. | `internal/llm/exercise_models_test.go`, `internal/api/routes/routing_admin_exercise_test.go`, `cmd/models/models_test.go`, live `models exercise --intent TOOL_USE moonshot/kimi-k2-turbo-preview` proof | L1/L2/L4 |
| R-BENCH-RCA-2 | Verifier retry suppression after substantive tool progress cannot finalize an empty assistant response under an `ok` turn. If final prose is blank but authoritative tool observations exist, the pipeline preserves observed tool evidence and records `response_synthesized_from_observation` in RCA. | `internal/pipeline/pipeline_run_test.go::TestPipeline_Run_SuppressedVerifierRetryDoesNotStoreBlankAfterToolProgress` | L1/L2 |
| R-BENCH-RCA-3 | Benchmark CLI progress previews are bounded single-line telemetry: multiline answers, fenced code blocks, tabs, carriage returns, ANSI/control sequences, and Markdown fence delimiters cannot bleed into the next progress row while full raw content remains persisted for RCA/rescoring. | `cmd/models/models_test.go::TestFormatExercisePreviewSingleLineSafe` | L1 |
| R-BENCH-RCA-4 | Transport/API failures and provider timeouts remain persisted benchmark/RCA incidents but do not count as exercised efficacy coverage, do not overwrite older valid intent evidence, do not render as `7/7` all-zero model-comparison evidence, and legacy rows with blank `result_class` plus `error_msg` or ambiguous blank/zero/no-class failures are reclassified/excluded during scorecard/rescore handling. | `internal/llm/exercise_models_test.go::TestExerciseModels_TransportErrorsAreValidityOnlyEvidence`, `internal/db/exercise_repo_test.go::TestExerciseScorecard_ExcludesValidityOnlyFailures`, `internal/db/exercise_repo_test.go::TestExerciseScorecard_ExcludesLegacyBlankZeroFailures`, `internal/api/routes/routing_admin_exercise_test.go::TestRescoreExerciseResults_ClassifiesLegacyTransportErrors`, `internal/api/routes/routing_admin_exercise_test.go::TestRescoreExerciseResults_ClassifiesLegacyBlankZeroFailuresAsAmbiguous`, `cmd/models/models_test.go::TestBuildComparisonRows_DoesNotOverwriteHistoricalEvidenceWithValidityOnlyFreshRun`, `cmd/models/models_test.go::TestBuildComparisonRows_FreshValidityOnlyRunDoesNotCreateEvidenceCoverage` | L1/L2 |
| R-BENCH-RCA-5 | Evaluable benchmark rows below the default `0.50` quality floor are classified as quality-gate failures, not passes, while their actual quality remains included in aggregate efficacy and rescore pass/fail decisions. | `internal/llm/exercise_models_test.go::TestExerciseModels_LowQualityRowsFailButRemainEfficacyEvidence`, `internal/api/routes/routing_admin_exercise_test.go::TestRescoreExerciseResults_AppliesDefaultQualityFloor` | L1/L2 |
| R-BENCH-RCA-6 | Benchmark scope diagnosis supports exact prompt-row replay and explicit warm-up control so operators can investigate one inference such as `TOOL_USE:C2` without running a full intent slice or paying unintended local cold-start cost. | `internal/llm/exercise_models_test.go::TestExerciseModels_FiltersToExactPromptRow`, `internal/llm/exercise_models_test.go::TestExerciseModels_WarmupSkipBypassesLocalWarmup`, `internal/api/routes/routing_admin_exercise_test.go::TestExerciseModel_FiltersExactPromptRowAndSkipsWarmup`, `cmd/models/models_test.go` selector coverage, focused live row proof | L1/L2/L4 |
| R-BENCH-RCA-7 | Required-tool benchmark prompts score observed tool/result evidence separately from promissory intent, so answers that only say they will query/search/check do not pass `TOOL_USE:C2`-style rows. Legitimate negative results can pass lookup prompts, but access/configuration/tool failures that block the requested work do not pass `TOOL_USE:C3`-style rows. | `internal/llm/exercise_tool_use_test.go` | L1 |
| R-BENCH-RCA-8 | Action-oriented execution/delegation prompts do not score blocked or hypothetical “I would do this if tools were available” prose as clean completion. The response remains diagnostic evidence but fails the default pass floor unless it contains concrete completion evidence. | `internal/llm/exercise_action_completion_test.go` | L1 |
| R-BENCH-CODE-1 | Coding benchmark prompts that require a runnable artifact weight artifact correctness ahead of prose shape: the reverse-string prompt rewards fenced, parseable/type-checkable code with real reversal mechanics, executes deterministic input/output cases through bounded temporary harnesses for Go `string -> string`, Python `string -> string`, and Python in-place list mutation contracts, penalizes executable identity functions, and penalizes broken syntax below both. Wider evaluator support must be explicit and contract-bound. | `internal/llm/exercise_coding_test.go::TestScoreCoding_ReverseStringGoRewardsTypeCheckedFunctionalArtifact`, `internal/llm/exercise_coding_test.go::TestScoreCoding_ReverseStringGoUsesBoundedInputOutputExecution`, `internal/llm/exercise_coding_test.go::TestScoreCoding_ReverseStringPythonUsesBoundedInputOutputExecution`, `internal/llm/exercise_coding_test.go::TestScoreCoding_ArtifactBearingPromptPrefersRunnableArtifact`, `internal/llm/exercise_coding_test.go::TestScoreCoding_ArtifactBearingPromptPenalizesBrokenGoSyntax` | L1/L2 |
| R-GUARD-CANNED-1 | Guard exhaustion does not have canned user-facing fallback templates compiled into the runtime. Exhausted guards must return structured failure, retry through scoped corrective guidance, or preserve authoritative observed evidence; deterministic prose fallback maps are dead-code regressions. | `rg "guardFallbackTemplates|fallbackResponse|GetFallbackTemplate" internal/pipeline -g'*.go'` release audit, `go test ./internal/pipeline -count=1` | L1/A |
| R-GUARD-ABC-1 | ABC paper evaluation is active release work: any adopted guardrail replacement must express preconditions, hard/soft invariants, governance policy, recovery behavior, satisfaction/recovery metrics, and drift evidence explicitly enough for RCA and benchmark fitness. No ABC-inspired mechanism may reintroduce canned fallback prose or unverifiable confidence claims. | architecture review + future contract fitness tests before implementation | A |
