...
| R-BEH-03 | Persistent-artifact authoring turns privilege artifact-writing tools over authority-write tools, and false “created note/file/document” claims are retried unless the turn carries matching artifact-writing evidence | `internal/pipeline/turn_policy_test.go`, `internal/pipeline/guards_truthfulness_test.go`, focused live authoring proof | L1/L4 |
| R-BEH-04 | Focused single-step authoring turns use a capability-scoped tool profile instead of inheriting the generic operational always-include set, so the selected request surface collapses to artifact-writing plus only the minimum runtime/retrieval context actually justified by the turn | `internal/pipeline/turn_policy_test.go`, focused live authoring proof | L1/L4 |
...
| R-BEH-06 | Once a side-effecting tool call has already succeeded in a turn, duplicate replays of the same tool+arguments are suppressed from one central tool-semantics policy instead of being executed again and risking non-idempotent side effects | `internal/agent/loop_test.go` | L1/L4 |
...
| R-RT-21 | Intent-capability evidence uses the same canonical model key across seeded baselines, imported exercise observations, live routing observations, and recommendation callouts, so exercised models are not falsely labeled `unexercised` for TOOL_USE because of namespace drift | `internal/llm/profile_test.go`, `internal/llm/role_routing_test.go`, `internal/llm/routing_trace_test.go` | L1/L2 |
