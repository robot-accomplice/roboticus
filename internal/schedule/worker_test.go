...
func TestLease_Contention_ExactlyOneWinner(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			w := NewCronWorker(store, fmt.Sprintf("c-%d", idx), time.Second, nopExecutor())
...
func TestRetry_ExponentialBackoff(t *testing.T) {
	...
	w := NewCronWorker(store, "retrier", time.Second, nopExecutor())
	now := time.Now()
...
func TestRunRecording_SuccessAndFailure(t *testing.T) {
	...
	w := NewCronWorker(store, "recorder", time.Second, nopExecutor())
