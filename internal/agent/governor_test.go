...
func TestRateLimiter_Allow(t *testing.T) {
	...
	decision := r.Check(5, 10000, 0.10)
	if decision != GovernorAllow {
		t.Errorf("expected Allow, got %v", decision)
	}
}

func TestRateLimiter_DenyTurns(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{
	...
	decision := r.Check(11, 5000, 0.05)
	if decision != GovernorDeny {
		t.Errorf("expected Deny on turns, got %v", decision)
	}
}

func TestRateLimiter_DenyTokens(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{
		MaxTurnsPerSession:  25,
	...
	decision := r.Check(5, 60000, 0.10)
	if decision != GovernorDeny {
		t.Errorf("expected Deny on tokens, got %v", decision)
	}
}

func TestRateLimiter_DenyCost(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{
		MaxTurnsPerSession:  25,
		MaxTokensPerSession: 100000,
	...
	decision := r.Check(5, 10000, 0.60)
	if decision != GovernorDeny {
		t.Errorf("expected Deny on cost, got %v", decision)
	}
}

func TestRateLimiter_ThrottleNearLimit(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{
		MaxTurnsPerSession:  10,
		MaxTokensPerSession: 100000,
		MaxCostPerSession:   1.0,
...
func TestRateLimiter_ZeroConfig(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{}) // zero values = no limits
	decision := r.Check(1000, 1000000, 100.0)
	if decision != GovernorAllow {
		t.Errorf("zero config should always allow, got %v", decision)
	}
}

func TestRateLimiter_CooldownTracking(t *testing.T) {
	r := NewRateLimiter(RateLimiterConfig{
		MaxTurnsPerSession:  25,
		MaxTokensPerSession: 100000,
		MaxCostPerSession:   1.0,
	...
	r.lastError = time.Now().Add(-2 * time.Second)
	if r.InCooldown() {
