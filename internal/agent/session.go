...
// Session holds the state of an ongoing agent conversation.
type Session struct {
	ID           string
	AgentID      string
	AgentName    string
...
func NewSession(id, agentID, agentName string) *Session {
	return &Session{
		...
		AgentID:   agentID,
		AgentName: agentName,
		Authority: core.AuthorityExternal, // default: least privilege
	}
}
...
// PendingToolCalls returns tool calls that haven't been resolved yet.
