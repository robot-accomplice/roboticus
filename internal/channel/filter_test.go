...
func TestMentionFilter(t *testing.T) {
	f := &MentionFilter{BotName: "Goboticus"}
	if !f.ShouldProcess(&InboundMessage{Content: "hey @Goboticus help"}) {
		t.Error("should match mention")
	}
	if !f.ShouldProcess(&InboundMessage{Content: "goboticus please"}) {
...
func TestDefaultAddressabilityChain(t *testing.T) {
	chain := DefaultAddressabilityChain("Goboticus")
