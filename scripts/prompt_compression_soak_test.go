...
func TestPromptCompressionSoakScript_PinsQualityGate(t *testing.T) {
	...
	required := []string{
		...
		`base_url={lane_url}`,
		`scenarios={env['SOAK_SCENARIOS']}`,
		`env.setdefault("SOAK_CLEAR_CACHE", "1")`,
		`env.setdefault("SOAK_BYPASS_CACHE", "1")`,
		`LANE_HEARTBEAT_SECONDS`,
		`SOAK_PORT_FREE_TIMEOUT_SECONDS`,
		`wait_for_port_free(`,
		`port {host}:{port} still busy after {timeout_s}s`,
		`subprocess.Popen(`,
		`bufsize=1,`,
		`stdout=subprocess.PIPE,`,
		`stderr=subprocess.PIPE,`,
		`lane={label} heartbeat `,
		`underlying soak timed out after`,
		`if _raw_lane_timeout else 7200`,
