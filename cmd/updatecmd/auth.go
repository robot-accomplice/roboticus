...
func runAPIKeyLogin(provider string) error {
	...
	req, _ := http.NewRequest("PUT", cmdutil.APIBaseURL()+"/api/providers/"+provider+"/key", strings.NewReader(string(payload)))
...
var authLogoutCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		provider := args[0]
		if err := cmdutil.APIDelete("/api/providers/" + provider + "/key"); err != nil {
			return err
		}
