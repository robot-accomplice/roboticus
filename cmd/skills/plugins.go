...
var pluginsListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIGet("/api/plugins")
		if err != nil {
			return err
		}
...
Otherwise, SOURCE is treated as a catalog name.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if strings.HasSuffix(source, ".tar.gz") || strings.HasSuffix(source, ".zip") || strings.HasSuffix(source, ".ic.zip") {
			...
			data, err := cmdutil.APIPost("/api/plugins/install", payload)
			if err != nil {
				return err
			}
		...
		data, err := cmdutil.APIPost("/api/plugins/catalog/install", map[string]any{
			"name": source,
		})
		if err != nil {
			return err
...
var pluginsUninstallCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		_, err := cmdutil.APIPost("/api/plugins/"+name+"/disable", nil)
		if err != nil {
...
var pluginsEnableCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIPost("/api/plugins/"+args[0]+"/enable", nil)
		if err != nil {
			return err
		}
...
var pluginsDisableCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIPost("/api/plugins/"+args[0]+"/disable", nil)
		if err != nil {
			return err
		}
...
var pluginsSearchCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		registryURL := updatecmd.ResolveRegistryURL(cmdutil.EffectiveConfigPath())
		manifest, err := updatecmd.FetchRegistryManifest(context.Background(), registryURL)
		if err != nil {
		...
		var results []updatecmd.PluginCatalogEntry
		...
		cmdutil.PrintJSON(map[string]any{"plugins": results})
		return nil
	},
}
...
func checkPluginDependencies(payload map[string]any) {
	...
	plugins, err := cmdutil.APIGet("/api/plugins")
	if err != nil {
