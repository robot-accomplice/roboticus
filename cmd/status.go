...
var statusCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		agentStatus := "unknown"
		model := ""
		agent, agentErr := apiGet("/api/agent/status")
		if agentErr == nil {
		...
		// Sessions.
		if sessions, err := apiGet("/api/sessions"); err == nil {
			count := 0
		...
		// Skills.
		if skills, err := apiGet("/api/skills"); err == nil {
			enabled := 0
			total := 0
		...
		// Cron jobs.
		if cron, err := apiGet("/api/cron/jobs"); err == nil {
			total := 0
			failed := 0
		...
		// Cache stats.
		if cache, err := apiGet("/api/stats/cache"); err == nil {
			hitRate := 0.0
		...
		// Wallet balance.
		if wallet, err := apiGet("/api/wallet/balance"); err == nil {
			balance := "0.00"
			token := "USDC"
		...
		// Channels.
		if chData, err := apiGet("/api/channels/status"); err == nil {
