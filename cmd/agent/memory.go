...
var memoryEpisodicCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIGet("/api/memory/episodic")
		if err != nil {
			return err
		}
		cmdutil.PrintJSON(data)
...
var memorySearchCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIGet(fmt.Sprintf("/api/memory/search?q=%s", args[0]))
		if err != nil {
			return err
		}
		cmdutil.PrintJSON(data)
...
var memoryStatsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		data, err := cmdutil.APIGet("/api/stats/memory-analytics")
		if err != nil {
			return err
		}
...
var memoryReindexCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		data, err := cmdutil.APIPost("/api/memory/reindex", nil)
		if err != nil {
			return err
		}
		cmdutil.PrintJSON(data)
